 ...
 */
export function applySessionListFrame(sessions, frame) {
    if (frame.type === 'upsert') {
        ...
        if (sameSession(sessions[i], merged))
            return sessions;
    ...
    if (frame.type === 'delete') {
        if (!sessions.some(s => s.session_id === frame.session_id))
            return sessions;
 ...
 */
export function sessionListMustReseed(resume) {
...
export class SessionListStore {
    ...
    patch = (sessionId, fields) => {
        ...
        if (sameSession(this.snapshot.sessions[i], merged))
            return;
            ...
            try {
                ...
                for await (const frame of stream) {
                    ...
                    if (frame.type === 'hello') {
                        ...
                        if (sessionListMustReseed(frame.resume))
                            await this.refresh();
                        continue;
                    }
    ...
    async seed() {
        try {
            ...
            const data = (await res.json()) ?? EMPTY;
...
export function sharedSessionList(fetchFn, basePath) {
...
export function useSharedSessionList(store) {
    return useSyncExternalStore(store.subscribe, store.getSnapshot, store.getSnapshot);
}
//# sourceMappingURL=sessionListStore.js.map