...
import { createSSEEventBatcher, isDeferrableEventType } from './sseEventBatching';
...
export function applyEventToRows(rows, ev) {
    const evId = eventIdOf(ev);
...
export async function controlRefusal(res) {
    if (res.ok)
        return null;
...
export function projectServerSessionState(session) {
    if (session.state === 'running')
        return 'tool_running';
    if (session.state === 'waiting_on_approval')
...
export function useBridgeSession() {
    ...
    const setActivityIfChanged = useCallback((next) => {
    ...
    const eventBatcher = useRef(null);
    ...
    const getSessionUIState = useCallback((session) => projectServerSessionState(session), []);
    // --- SSE connection ---
    ...
    const startSSE = useCallback((sessionId) => {
        ...
        function handleSSEEvent(event) {
            ...
            if (isDeferrableEventType(type))
                eventBatcher.current.push(event);
            else
                eventBatcher.current.pushAndFlush(event);
            switch (type) {
                case 'stream': {
            ...
            if (!sessionsRef.current.find(s => s.session_id === id)?.info) {
                try {
                    ...
                    if (res.ok && activeSessionIdRef.current === id) {
                        ...
                        if (full?.info) {
                            setSessions(prev => prev.map(s => s.session_id === id ? { ...s, info: full.info } : s));
                        }
                    }
                }
                catch {
    ...
    // mode to mint one.
    const refreshAttachToken = useCallback(async (sessionId) => {
        try {
    ...
    // active session.
    const send = useCallback(async (text, explicitSessionId) => {
