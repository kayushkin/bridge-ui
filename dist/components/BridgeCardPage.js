...
export function BridgeCardPage() {
    ...
    const { fetch: fetchFn, kanbanStoreBasePath, noteboardBasePath, mailBasePath, mailPagePath, routes } = useBridgeConfig();
    ...
    const loadAssignments = useCallback(async () => {
        ...
        if (!res.ok)
            throw new Error(`assignments HTTP ${res.status}`);
        const raw = await res.json();
        return Array.isArray(raw) ? raw : [];
    }, [fetchFn, kanbanStoreBasePath, cardId]);
    const load = useCallback(async (reason = 'first') => {
        if (!cardId || !kanbanStoreBasePath || !noteboardBasePath)
            return;
        ...
        try {
            ...
            const loadedItem = await itemRes.json();
            ...
            const rawPlacements = placementsRes.ok ? await placementsRes.json() : null;
    ...
    const afterAssignmentWrite = useCallback(async (outcome) => {
    ...
    if (state === 'loading')
        return _jsx("div", { className: "bi-empty", children: "Loading card\u2026" });
    if (state === 'missing')
        return _jsxs("div", { className: "bi-empty", children: ["No card with id ", cardId, "."] });
    if (state === 'error')
        return _jsx("div", { className: "bi-empty", children: "Could not load this card." });
    if (!item)
        return _jsxs("div", { className: "bi-empty", children: ["No card with id ", cardId, "."] });
    ...
    _jsx("div", { className: "bk-drawer bk-card-page", children: _jsx(CardDetail, { card: card, boardID: card.placement.board_id, entityTypes: k.entityTypes, onClose: close, onPatch: async (patch) => {
                ...
                if (ok)
                    setLinks(await listCardLinks(cardId));
                return ok;
            }, onAssign: principalID => afterAssignmentWrite(k.assign(cardId, principalID)), onUnassign: principalID => afterAssignmentWrite(k.unassign(cardId, principalID)), onOpenChat: link => navigate(`${routes.chat}?session=${encodeURIComponent(link.ref)}`), onOpenInMail: (accountID, messageID) => {
            ...
            }, mailBasePath: mailBasePath, fetchFn: fetchFn }) }));
}
//# sourceMappingURL=BridgeCardPage.js.map