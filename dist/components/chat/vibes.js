...
export const VIBES = ['danger', 'warning', 'question', 'action', 'info', 'descriptive'];
/** Severity, derived from `VIBES`' order. Higher wins a section (see `assignVibes`). */
const VIBE_RANK = Object.fromEntries(VIBES.map((vibe, index) => [vibe, VIBES.length - index]));
...
const ATTENTION_VIBES = ['danger', 'warning', 'question', 'action'];
...
const RULES = [
    {
        ...
        test: (_block, firstLine) => /^(?:🚨|❌|\*{0,2}(?:error|failed|failure|broken|fatal)\b)/i.test(firstLine),
    },
    {
        name: 'warning-opener',
        vibe: 'warning',
        test: (_block, firstLine) => /^(?:⚠|\*{0,2}(?:note|caveat|careful|caution|heads up|beware)\b)/i.test(firstLine),
    },
    {
        name: 'question-opener',
        vibe: 'question',
        test: (_block, firstLine) => /^\*{0,2}(?:should i|shall i|do you want|would you like|do you prefer|which would you)\b/i.test(firstLine),
    },
    {
        name: 'question-mark',
        vibe: 'question',
...
export function classifyBlock(block) {
    ...
    if (alert)
        return ALERT_VIBE[alert[1].toLowerCase()] ?? 'warning';
    const firstLine = leadLine(source);
    for (const rule of RULES) {
        if (rule.test(block, firstLine, source))
            return rule.vibe;
    }
    return 'descriptive';
}
...
export function assignVibes(blocks) {
    ...
    const closeSection = (end) => {
        ...
        for (let i = sectionStart; i < end; i++) {
            const vibe = own[i];
            if (!ATTENTION_VIBES.includes(vibe))
                continue;
            if (!winner || VIBE_RANK[vibe] > VIBE_RANK[winner])
                winner = vibe;
        }
        if (!winner)
            return;
 ...
 *  make. */
export function runPositions(vibes) {
    return vibes.map((vibe, index) => {
        ...
        if (continuesFromAbove && continuesBelow)
            return 'mid';
...
function collectBlocks(tree, source) {
    ...
    tree.children.forEach((node, index) => {
        if (INVISIBLE_TYPES.has(node.type))
            return;
