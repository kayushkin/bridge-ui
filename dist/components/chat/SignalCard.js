...
function CustomerReplyDraft({ draft, busy }) {
    const [body, setBody] = useState(draft.body);
    const [subject, setSubject] = useState(draft.subject);
    const recipient = draft.to || '';
    return (_jsxs("section", { className: "bc-signal-draft", "aria-label": "Drafted reply to the customer", children: [_jsxs("div", { className: "bc-signal-draft-header", children: [_jsx("span", { className: "bc-signal-draft-label", children: "Forward to the customer" }), _jsx("span", { className: "bc-signal-draft-hint", children: "drafted by triage \u2014 edit before sending" })] }), _jsxs("dl", { className: "bc-signal-draft-fields", children: [_jsx("dt", { children: "To" }), _jsx("dd", { children: recipient
                            ? _jsx("span", { className: "bc-signal-draft-to", children: recipient })
                            : _jsx("span", { className: "bc-signal-draft-unresolved", title: "The sender of the card's mail could not be read from mailstack, so no address was filled in.", children: "recipient unresolved" }) }), _jsx("dt", { children: "Subject" }), _jsx("dd", { children: _jsx("input", { className: "bc-signal-draft-subject", value: subject, disabled: busy, onChange: e => setSubject(e.target.value) }) })] }), _jsx("textarea", { className: "bc-signal-draft-body", value: body, disabled: busy, rows: Math.min(10, Math.max(3, body.split('\n').length + 1)), onChange: e => setBody(e.target.value) }), _jsxs("div", { className: "bc-signal-actions", children: [_jsx("button", { type: "button", className: "bc-signal-reply", disabled: true, title: "Sending is not wired yet: mailstack has no send route. Copy the draft into your mail client for now.", children: "Reply to customer" }), _jsx("span", { className: "bc-signal-draft-stub", children: "not wired to send yet" })] })] }));
}
