...
import SessionStatusLine, { STATUS_SPACER_CLASSNAME, useSessionStatus, } from './SessionStatusLine';
...
const REMARK_PLUGINS = [remarkGfm, remarkRefChips, remarkVibes];
 ...
 *  re-derivation here. */
export default function TurnList({ sessionId, view, markdown, streaming, compacting, composerStatus, style, }) {
    const { turns, entries, sourceGroups, visibleEntryIds, sourcesFor, loading, more, loadOlder } = useTurns(sessionId, view);
    ...
    const lastStatusKind = useRef(null);
    ...
    // per streamed token.
    const [closedForEntries, setClosedForEntries] = useState(null);
    useEffect(() => {
    ...
    const agentName = (summary?.harness ? harnessMap.get(summary.harness)?.label : undefined) ||
        summary?.harness ||
        'assistant';
    ...
    const sourceNamesByGroupId = useMemo(() => {
        ...
        for (const [groupId, names] of Object.entries(sourceGroups ?? {}))
            byGroup.set(groupId, [...names]);
        for (const entry of Object.values(entries)) {
            ...
            if (!names.includes(entry.source))
    ...
    const sourceNamesForEntries = useCallback((entryIds) => {
        const names = [];
    ...
    const onActivateSessionRef = useCallback((_kind, refId) => select(refId), [select]);
    ...
    const items = useMemo(() => {
        ...
        if (view === 'raw') {
            return turns.map((turn, idx) => (_jsx(TurnItem, { turn: turn, entries: entries, entryIds: visibleEntryIds(turn.id), view: view, markdown: markdown, agentName: agentName, sourcesFor: sourcesFor, sourceNamesForEntries: sourceNamesForEntries, onActivateSessionRef: onActivateSessionRef, resultedToolIds: resultedToolIds, streaming: streaming && idx === turns.length - 1 }, turn.id)));
        }
        ...
        const flushSpan = () => {
            if (span.length === 0)
                return;
            out.push(_jsx(SpanRow, { segments: span, agentName: agentName, streaming: spanStreaming, markdown: markdown, sourceNamesForEntries: sourceNamesForEntries, onActivateSessionRef: onActivateSessionRef }, `span:${span[0].turnId}`));
            span = [];
            spanStreaming = false;
        };
        for (let t = 0; t < turns.length; t++) {
            ...
            const built = buildTurnItems(entries, ids, streaming && provisionalWindowOpen && t === lastTurnIdx);
            let lastAssistantItem = null;
            for (let i = built.length - 1; i >= 0; i--) {
                if (built[i].kind === 'assistant') {
                    lastAssistantItem = built[i];
                    break;
                }
            }
            ...
            if (t === 0 || (opener && opener.kind === 'user'))
                flushSpan();
            let seg = null;
            for (const it of built) {
                if (it.kind === 'user') {
                    flushSpan();
                    seg = null;
                    out.push(_jsx(UserTurnRow, { item: it, sourceNames: sourceNamesForEntries(it.entryIds) }, it.key));
                    continue;
                }
    ...
    const settledSession = useRef(null);
    useEffect(() => {
        if (!sessionId)
            return;
        const h = listRef.current;
        if (!h || items.length === 0 || lastChildIndex < 0)
            return;
    ...
    if (!sessionId) {
        ...
        return (_jsx("div", { ...rootProps, children: _jsx("div", { className: "bc-turns-body", children: _jsx("div", { className: "bc-turns-empty", children: pending
    ...
    const emptyStateStatus = statusVisible ? (_jsx(SessionStatusLine, { status: status, onOpenSession: select })) : null;
    if (loading && turns.length === 0) {
        return (_jsxs("div", { ...rootProps, children: [_jsx("div", { className: "bc-turns-body", children: _jsx("div", { className: "bc-turns-empty", children: "Loading\u2026" }) }), emptyStateStatus] }));
    }
    if (turns.length === 0) {
        return (_jsxs("div", { ...rootProps, children: [_jsx("div", { className: "bc-turns-body", children: _jsx("div", { className: "bc-turns-empty", children: "No messages yet" }) }), emptyStateStatus] }));
    }
    return (_jsxs("div", { ...rootProps, 
        ...
        // rest of the time.
        onWheel: lingerSpacer ? clearLingerSpacer : undefined, onTouchMove: lingerSpacer ? clearLingerSpacer : undefined, children: [markdown && _jsx(VibeLegend, {}), _jsx(ToolContext.Provider, { value: toolContextValue, children: _jsxs(VList, { ref: listRef, className: "bc-turns-body", onScroll: onScroll, children: [more ? (_jsx("button", { className: styles.loadOlder, onClick: loadOlder, children: "\u2191 Load older messages" }, "__older__")) : null, items] }) }), statusVisible ? (_jsx(SessionStatusLine, { status: status, onOpenSession: select })) : lingerSpacer ? (_jsx("div", { className: STATUS_SPACER_CLASSNAME, "aria-hidden": true })) : null, !atBottom && (_jsx("button", { className: "bc-jump-latest", onClick: jumpToLatest, title: "Jump to latest", children: "\u2193 New messages" }))] }));
}
...
function buildTurnItems(entries, entryIds, 
...
turnStreaming = false) {
    ...
    for (const id of entryIds) {
        ...
        if (e.kind === 'text' ||
            ...
            switch (e.kind) {
                ...
                case 'result':
                    if (e.text)
                        g.results.push(e.text);
                    g.item.entryIds.push(e.id);
                    break;
                case 'thinking':
                    if (e.text)
                        g.thinkings.push(e.text);
                    g.item.entryIds.push(e.id);
                    break;
                case 'error':
    ...
    if (turnStreaming) {
        for (let i = out.length - 1; i >= 0; i--) {
            ...
            if (it.kind !== 'assistant')
                break; // user/marker after it: nothing open
    ...
    const perMessage = out.filter((it) => it.kind !== 'assistant' || it.prose || it.narration || it.thinking || it.errors.length > 0);
    ...
    for (const it of perMessage) {
        ...
        if (it.prose)
            block.prose = block.prose ? `${block.prose}\n\n${it.prose}` : it.prose;
        if (it.thinking)
            block.thinking = block.thinking ? `${block.thinking}\n\n${it.thinking}` : it.thinking;
        if (it.errors.length > 0)
            block.errors = [...block.errors, ...it.errors];
        if (it.usage)
            block.usage = it.usage;
        block.entryIds = [...block.entryIds, ...it.entryIds];
    }
    return merged;
}
const TurnItem = memo(function TurnItem({ turn, entries, entryIds, view, markdown, streaming, sourcesFor, onActivateSessionRef, resultedToolIds, }) {
    ...
    return (_jsxs("div", { className: `bc-turns-item ${roleClass} ${streaming ? 'bc-turns-streaming' : ''} ${styles.vrow}`, children: [_jsxs("div", { className: "bc-turns-meta", children: [_jsx("span", { className: "bc-turns-actor", children: turn.role === 'user' ? 'You' : turn.role }), _jsx("span", { className: "bc-turns-ts", children: formatHMS(turn.ts) }), streaming && _jsx("span", { className: "bc-turns-streaming-tag", children: "streaming\u2026" })] }), entryIds.map((id) => {
                const entry = entries[id];
                if (!entry)
                    return null;
                // compact_boundary is a horizontal marker, not a bubble.
                if (entry.kind === 'system' && entry.subtype === 'compact_boundary') {
                    return _jsx(CompactBoundary, { ts: entry.ts }, id);
                }
                ...
                return (_jsxs("div", { className: styles.entry, children: [_jsxs("div", { className: styles.rawMeta, children: ["#", entry.eventId, " \u00B7 ", entry.source, " \u00B7 ", entry.kind, " \u00B7 ", entry.role, entry.duplicate && _jsx("span", { className: styles.rawDupTag, children: "duplicate" }), entry.primary && _jsx("span", { className: styles.sourcePrimary, children: " primary" })] }), _jsx(EntryBody, { entry: entry, view: "raw", turnHasText: turnHasText, markdown: markdown, onActivateSessionRef: onActivateSessionRef, resultedToolIds: resultedToolIds }), entry.usage && _jsx(UsageLine, { usage: toBridgeUsage(entry.usage) }), sources.length > 1 && (_jsxs(_Fragment, { children: [_jsxs("button", { className: styles.sourcesBadge, onClick: () => toggleReveal(id), children: [sources.length, " sources: ", sourceLabel(sources), " ", isRevealed ? '▲' : '▼'] }), isRevealed && (_jsx("div", { className: styles.sourcesReveal, children: sources.map((copy) => (_jsxs("div", { className: styles.sourceCopy, children: [_jsxs("div", { className: styles.sourceMeta, children: ["#", copy.eventId, " \u00B7 ", copy.source, copy.primary && _jsx("span", { className: styles.sourcePrimary, children: " \u00B7 primary" }), copy.duplicate && _jsx("span", { className: styles.sourceDup, children: " \u00B7 duplicate" })] }), _jsx(EntryBody, { entry: copy, view: view, turnHasText: turnHasText, markdown: markdown, onActivateSessionRef: onActivateSessionRef, resultedToolIds: resultedToolIds })] }, copy.id + ':' + copy.eventId))) }))] }))] }, id));
            })] }));
});
...
function SourcesTag({ names }) {
    if (names.length < 2)
        return null;
    return (_jsxs("span", { className: styles.turnsSourcesBadge, "data-testid": "turns-sources-badge", title: `Reported by ${names.length} sources: ${names.join(', ')}. Per-copy detail is in the Raw view.`, children: [names.length, " sources: ", names.join(' + ')] }));
}
/** Distinct source labels for the badge, e.g. "harness + otel". */
function sourceLabel(sources) {
    const seen = [];
    for (const s of sources)
        if (!seen.includes(s.source))
            seen.push(s.source);
    return seen.join(' + ');
}
/** A context-compaction marker — the harness dropped older context. Mirrors
 *  bridge-ui's bc-turns-marker (compact variant). */
function CompactBoundary({ ts }) {
    return (_jsxs("div", { className: "bc-turns-marker bc-turns-marker-compact", role: "separator", children: [_jsx("span", { className: "bc-turns-marker-line", "aria-hidden": true }), _jsx("span", { className: "bc-turns-marker-text", children: "Context compacted" }), _jsx("span", { className: "bc-turns-marker-ts", children: formatHMS(ts) }), _jsx("span", { className: "bc-turns-marker-line", "aria-hidden": true })] }));
}
 ...
 */
function Aside({ variant, text, live = false, count, }) {
    const icon = variant === 'reasoning' ? '💭' : '💬';
    const label = variant === 'reasoning'
        ? 'Reasoning'
        : count && count >= 2
    ...
    useEffect(() => {
        ...
        if (next === null || el.open === next)
            return;
        changingOurselves.current = true;
        el.open = next;
    }, [live]);
    return (_jsxs("details", { ref: ref, className: cls, onToggle: () => {
        ...
        }, children: [_jsxs("summary", { children: [_jsx("span", { className: "bc-turns-aside-icon", "aria-hidden": true, children: icon }), _jsx("span", { children: label }), live && _jsx("span", { className: "bc-turns-aside-dots", "aria-hidden": true, children: "\u2026" })] }), _jsx("div", { className: "bc-turns-aside-text", children: text })] }));
}
/** One user prompt row.
 *
...
const UserTurnRow = memo(function UserTurnRow({ item, sourceNames, }) {
    return (_jsxs("div", { className: `bc-turns-item bc-turns-user ${styles.vrow}`, "data-entry-ids": item.entryIds.join(' '), children: [_jsxs("div", { className: "bc-turns-meta", children: [_jsx("span", { className: "bc-turns-actor", children: "You" }), _jsx("span", { className: "bc-turns-ts", children: formatHMS(item.ts) }), _jsx(SourcesTag, { names: sourceNames })] }), _jsx("div", { className: "bc-turns-text", children: item.text })] }));
});
...
function taskNotificationLabel(text) {
...
function NotificationChip({ text, ts }) {
    return (_jsxs("details", { className: `bc-turns-notification ${styles.notificationChip}`, children: [_jsxs("summary", { className: styles.notificationChipSummary, children: [_jsx("span", { "aria-hidden": true, children: "\u23F1" }), _jsx("span", { className: styles.notificationChipLabel, children: taskNotificationLabel(text) }), _jsx("span", { className: "bc-turns-ts", children: formatHMS(ts) })] }), _jsx("pre", { className: styles.entryPre, children: text })] }));
}
...
function SegmentContent({ segment, live, markdown, onActivateSessionRef, }) {
    const lastAssistant = [...segment.items].reverse().find((it) => it.kind === 'assistant');
    return (_jsxs(_Fragment, { children: [segment.notification && _jsx(NotificationChip, { text: segment.notification, ts: segment.ts }), segment.items.map((it) => {
                if (it.kind === 'marker')
                    return _jsx(CompactBoundary, { ts: it.ts }, it.key);
                if (it.kind !== 'assistant')
                    return null;
                const itemLive = live && it === lastAssistant && !!(it.thinking || it.narration);
                return (_jsxs(Fragment, { children: [it.thinking && _jsx(Aside, { variant: "reasoning", text: it.thinking, live: itemLive }), it.narration && (_jsx(Aside, { variant: "narration", text: it.narration, live: itemLive, count: it.narrationCount })), it.prose && (_jsx(ProseBody, { text: it.prose, markdown: markdown, onActivateSessionRef: onActivateSessionRef })), it.errors.map((e) => (_jsx(EntryBody, { entry: e, view: "turns", turnHasText: true, markdown: markdown, onActivateSessionRef: onActivateSessionRef }, e.id)))] }, it.key));
            })] }));
}
...
function SpanRow({ segments, agentName, streaming, markdown, sourceNamesForEntries, onActivateSessionRef, }) {
    ...
    const isError = segments.some((seg) => seg.items.some((it) => it.kind === 'assistant' && it.errors.length > 0));
    const headerTs = segments[0].ts;
    return (_jsxs("div", { className: `bc-turns-item bc-turns-assistant${isError ? ' bc-turns-error' : ''}${streaming ? ' bc-turns-streaming' : ''} ${styles.vrow}`, "data-entry-ids": segments.flatMap((seg) => seg.items.flatMap((it) => (it.kind === 'assistant' || it.kind === 'user' ? it.entryIds : []))).join(' '), children: [_jsxs("div", { className: "bc-turns-meta", children: [_jsx("span", { className: "bc-turns-actor", children: agentName }), _jsx("span", { className: "bc-turns-ts", children: formatHMS(headerTs) }), lastAssistant?.usage && _jsx(UsageLine, { usage: toBridgeUsage(lastAssistant.usage) }), _jsx(SourcesTag, { names: sourceNamesForEntries(lastAssistant?.entryIds ?? []) }), asideLive && lastAssistant?.thinking && (_jsx("span", { className: "bc-turns-aside-tag bc-turns-aside-reasoning", children: "reasoning\u2026" })), asideLive && lastAssistant?.narration && (_jsx("span", { className: "bc-turns-aside-tag bc-turns-aside-narration", children: "narration\u2026" })), streaming && !asideLive && _jsx("span", { className: "bc-turns-streaming-tag", children: "streaming\u2026" })] }), intermediate.length > 0 && (_jsxs("details", { className: `bc-turns-progress ${styles.progressGroup}`, children: [_jsxs("summary", { className: styles.progressGroupSummary, children: [_jsx("span", { "aria-hidden": true, children: "\u258D" }), _jsxs("span", { children: [intermediate.length, " progress report", intermediate.length === 1 ? '' : 's'] })] }), _jsx("div", { className: styles.progressGroupBody, children: intermediate.map((seg, i) => (_jsx(SegmentContent, { segment: seg, live: false, markdown: markdown, onActivateSessionRef: onActivateSessionRef }, `${seg.turnId}:${i}`))) })] })), _jsx(SegmentContent, { segment: final, live: streaming, markdown: markdown, onActivateSessionRef: onActivateSessionRef })] }));
}
...
function ProseBody({ text, markdown, onActivateSessionRef, }) {
    ...
    return (_jsx("div", { className: `bc-turns-text bc-turns-md ${styles.vibeProse}`, children: _jsx(ReactMarkdown, { remarkPlugins: REMARK_PLUGINS, components: {
                'ref-chip': (props) => (_jsx(RefChip, { ...props, className: styles.refChip, onActivate: onActivateSessionRef })),
            }, children: text }) }));
}
 ...
 *  the audit surface. */
function EntryBody({ entry, view, turnHasText, markdown, onActivateSessionRef, resultedToolIds, }) {
    switch (entry.kind) {
        case 'text':
            return (_jsxs(_Fragment, { children: [_jsx(ProseBody, { text: entry.text ?? '', markdown: markdown, onActivateSessionRef: onActivateSessionRef }), entry.recovered && (_jsx("span", { className: styles.recoveredMarker, children: "\u21BB recovered after stream interruption" }))] }));
        case 'error': {
            const isTerminal = entry.code ? TERMINAL_ERROR_CODES.has(entry.code) : false;
            const chipClass = entry.retryable && !isTerminal ? styles.errorChipRetryable : styles.errorChipTerminal;
            return (_jsxs("div", { className: `${styles.errorChip} ${chipClass}`, children: [_jsx("span", { "aria-hidden": true, children: "\u26A0" }), entry.code && _jsx("span", { className: styles.errorChipCode, children: entry.code }), _jsx("span", { children: entry.text ?? 'error' }), entry.statusCode ? (_jsxs("span", { className: styles.errorChipStatus, children: ["\u00B7 ", entry.statusCode] })) : null, entry.retryable ? _jsx("span", { className: styles.errorChipStatus, children: "\u00B7 retryable" }) : null] }));
        }
        case 'thinking':
            return _jsx(Aside, { variant: "reasoning", text: entry.text ?? '' });
        case 'tool_call':
            ...
            return (_jsx(ToolItem, { tool: toToolEvent(entry), running: isToolRunning(entry, resultedToolIds ?? EMPTY_TOOL_IDS) }));
        case 'tool_result':
            ...
            return (_jsxs("div", { className: styles.entryTool, children: [_jsx("span", { className: styles.entryToolName, children: entry.toolName ?? 'result' }), entry.toolResult !== undefined && (
                    ...
                    _jsx("pre", { className: styles.entryPre, children: typeof entry.toolResult === 'string'
                            ? entry.toolResult
        ...
        case 'result':
            ...
            if (view === 'turns') {
                return !turnHasText && entry.text ? (_jsx(ProseBody, { text: entry.text, markdown: markdown, onActivateSessionRef: onActivateSessionRef })) : null;
            }
            return (_jsxs(_Fragment, { children: [entry.text ? (_jsx(ProseBody, { text: entry.text, markdown: markdown, onActivateSessionRef: onActivateSessionRef })) : null, entry.raw !== undefined && _jsx("pre", { className: styles.entryPre, children: stringify(entry.raw) })] }));
        case 'system':
            ...
            if (entry.subtype && entry.subtype.includes('narration')) {
                return _jsx(Aside, { variant: "narration", text: entry.text ?? '' });
            }
            if (entry.text)
                return _jsx("div", { className: "bc-turns-text", children: entry.text });
            return null;
        default:
            ...
            if (entry.raw !== undefined)
                return _jsx("pre", { className: styles.entryPre, children: stringify(entry.raw) });
            return null;
    }
}
...
//# sourceMappingURL=TurnList.js.map