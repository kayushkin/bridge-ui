...
export default function SignalsInbox({ onSelectSession, }) {
    ...
    const sessionIds = useMemo(() => (open ? ordered.map(request => request.sessionId) : []), [open, ordered]);
    ...
    if (!available || (waiting === 0 && error === null))
        return null;
    return (_jsxs("div", { className: styles.inbox, role: "region", "aria-label": "Signals needing you", children: [_jsxs("button", { type: "button", className: styles.inboxHeader, "aria-expanded": open, onClick: toggle, children: [_jsx("span", { className: styles.inboxCaret, "aria-hidden": true, children: open ? '▾' : '▸' }), _jsx("span", { className: styles.inboxTitle, children: "Needs you" }), _jsx("span", { className: styles.inboxCount, children: waiting })] }), error !== null && _jsxs("p", { className: styles.inboxError, children: ["Couldn\u2019t load: ", error] }), open && (_jsx("div", { className: styles.inboxBody, children: ordered.map(request => (_jsx(SignalRequestCard
                ...
                , { request: request, compact: true, onResolved: reload, 
                    ...
                    allowDismissWithoutAnswer: true, header: _jsx("button", { type: "button", className: styles.inboxSessionButton, title: "Open this session", onClick: () => onSelectSession(request.sessionId), children: sessionName(request.sessionId) }) }, request.requestId || request.signals[0]?.id))) }))] }));
}
/**
 ...
 * all.
 */
export function questionsFirst(requests) {
    const holdsQuestion = (request) => request.signals.some(signal => signal.kind === SIGNAL_KIND_QUESTION);
...
//# sourceMappingURL=SignalsInbox.js.map