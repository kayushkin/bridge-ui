...
export function SessionPermissionMode({ session, harnesses, }) {
    ...
    return (_jsx("div", { className: "bc-ctrl-mode", title: MODE_TITLES[mode] ?? '', children: _jsx("select", { className: "bc-ctrl-mode-select", value: mode, disabled: busy, onChange: e => handleChange(e.target.value), "aria-label": "Permission mode", children: supported.map(m => (_jsx("option", { value: m, children: MODE_LABELS[m] ?? m }, m))) }) }));
}
