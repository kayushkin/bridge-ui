export function findLeafPath(node, id) {
    if (!node)
        return null;
    if (node.kind === 'leaf')
        return node.workspaceId === id ? [] : null;
...
export function* iterateLeafIds(node) {
    if (!node)
        return;
    if (node.kind === 'leaf') {
        yield node.workspaceId;
        return;
    }
    for (const c of node.children)
        yield* iterateLeafIds(c);
}
export function firstLeafId(node) {
    for (const id of iterateLeafIds(node))
        return id;
    return null;
}
export function splitLeaf(tree, targetId, newWorkspaceId, direction, position = 'after') {
    const newLeaf = { kind: 'leaf', workspaceId: newWorkspaceId };
    ...
    if (!path)
        return tree;
    const orderedChildren = (existing) => position === 'before' ? [newLeaf, existing] : [existing, newLeaf];
...
export function removeLeaf(tree, targetId) {
    if (!tree)
        return null;
    if (tree.kind === 'leaf')
        return tree.workspaceId === targetId ? null : tree;
    const path = findLeafPath(tree, targetId);
...
export function isLayoutValid(node, knownIds) {
    if (!node)
        return true;
    if (node.kind === 'leaf')
        return knownIds.has(node.workspaceId);
...
export function buildFlatLayout(workspaceIds) {
    if (workspaceIds.length === 0)
        return null;
    if (workspaceIds.length === 1)
        return { kind: 'leaf', workspaceId: workspaceIds[0] };
    return {
        kind: 'split',
        direction: 'h',
