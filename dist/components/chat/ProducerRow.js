...
export function ProducerRow({ apiFetch, producerBasePath, instances, sessions, onSelect, onNewChat }) {
    ...
    const openChat = useCallback(() => {
        ...
        if (existing)
            ...
            onNewChat(producerInstance.id, 'replace');
    ...
    const pct = cfg && cfg.week_limit_usd > 0
        ...
        : 0;
    return (_jsxs("div", { className: `bc-producer ${enabled ? 'bc-producer-enabled' : ''}`, children: [_jsxs("div", { className: "bc-session-item bc-producer-row", children: [_jsxs("button", { className: "bc-session-item-main", onClick: openChat, title: unreachable ? 'Producer service unreachable' : 'Open the Producer chat', children: [_jsx("span", { className: "bc-session-harness bc-producer-badge", "aria-hidden": true, children: "\uD83C\uDFAC" }), _jsx("span", { className: `bc-producer-dot ${unreachable ? 'off' : enabled ? 'on' : 'idle'}` }), _jsx("span", { className: "bc-session-label bc-producer-label", children: "Producer" })] }), _jsx("span", { className: "bc-session-menu-btn bc-producer-caret", role: "button", tabIndex: 0, onClick: () => setExpanded(x => !x), title: "Producer settings", children: expanded ? '▾' : '⚙' })] }), expanded && (_jsx("div", { className: "bc-producer-panel", children: unreachable ? (_jsxs("div", { className: "bc-producer-error", children: ["Producer offline (", error, ")"] })) : cfg ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "bc-producer-hint", children: "Click the row to open the chat." }), _jsx("button", { className: `bc-producer-enable ${enabled ? 'on' : 'off'}`, disabled: busy, onClick: () => patch({ enabled: !enabled }), title: "Autonomous mode: lets the Producer run scheduled sweeps and ping you. Chatting works either way.", children: enabled ? '● Autonomous mode on — click to disable' : '○ Enable autonomous mode' }), _jsxs("label", { className: "bc-producer-limit", children: [_jsx("span", { children: "Weekly limit\u00A0$" }), _jsx("input", { type: "number", min: "0", step: "1", value: limitDraft, disabled: busy, onChange: e => setLimitDraft(e.target.value), onBlur: saveLimit, onKeyDown: e => { if (e.key === 'Enter')
