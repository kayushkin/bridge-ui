...
import { SessionSettingsInline, SessionSettingsPanel, useSessionSettings, } from './SessionSettings';
...
function Workspace() {
    ...
    const deeplink = useRef(initialSessionDeeplinkState);
    ...
    useEffect(() => {
        if (bootstrapped.current || !target.ready)
            return;
        bootstrapped.current = true;
        if (activeId)
            return;
    ...
    const sidebar = (_jsx(Sidebar, { newTarget: target, onToggleCollapse: toggleSidebar, onAfterSelect: minimal ? () => setDrawerOpen(false) : undefined }));
    return (
    ...
    // no CSS.
    _jsxs("div", { className: `bc-container ${minimal ? 'bc-minimal' : ''}`, children: [minimal && (_jsx(MinimalTopBar, { title: activeSummary?.displayName || activeSummary?.sessionId || '' })), _jsxs("div", { className: "bc-main", children: [!minimal && (sidebarCollapsed ? (_jsxs("button", { className: "bc-sidebar-strip", onClick: toggleSidebar, title: "Show sessions", "aria-label": "Show sessions", children: [_jsx("span", { className: "bc-sidebar-strip-chevron", "aria-hidden": true, children: "\u25B8" }), _jsx("span", { className: "bc-sidebar-strip-label", children: "Sessions" })] })) : sidebar), _jsx("div", { className: "bc-workspaces", children: _jsx(ThreadPane, { newTarget: target }) })] }), minimal && (_jsxs(_Fragment, { children: [_jsx(SessionDrawer, { children: sidebar }), _jsx(ChromeSheet, {})] })), showReengage && (_jsx("button", { type: "button", className: "bc-mc-reengage", onClick: () => setOverride(null), "aria-label": "Switch to mobile layout", children: "Use mobile layout" }))] }));
}
...
function ThreadPane({ newTarget }) {
    ...
    const setPaneSizes = useCallback((next) => {
        savePaneSizes(next);
    ...
    const setMarkdown = useCallback((next) => {
    ...
    const [composerFailedAction, setComposerFailedAction] = useState(null);
    ...
    const paneStyle = (key) => ({ flex: `${paneSizes[key]} 1 0` });
    ...
    const settingsSheet = (_jsxs("div", { className: "bc-controls-bar", children: [_jsx(SessionSettingsInline, { settings: settings, controls: controls }), _jsx(SessionSettingsPanel, { settings: settings, controls: controls })] }));
    return (
    ...
    _jsxs("div", { className: `bc-workspace bc-workspace-focused ${minimal ? 'bc-workspace-minimal' : ''}`, children: [minimal ? (
            ...
            _jsx(MinimalPaneSwitch, { pane: mobilePane, onPick: setMobilePane })) : null, _jsx(SessionHeader, { panesHidden: panesHidden, paneDrawable: paneDrawable, togglePane: togglePane, raw: raw, setRaw: setRaw, markdown: markdown, setMarkdown: setMarkdown, settings: settings, controls: controls }), _jsxs("div", { className: "bc-chat-split", ref: splitRowRef, children: [panes.map((pane, index) => (_jsxs(Fragment, { children: [index > 0 && (_jsx(SplitDragHandle, { axis: "horizontal", className: "bc-split-resizer", resolveDraggedPair: () => {
                                    const row = splitRowRef.current;
                                    if (!row)
                                        return null;
                                    const before = panes[index - 1];
                                    const elementBefore = row.querySelector(`[data-pane="${before}"]`);
                                    const elementAfter = row.querySelector(`[data-pane="${pane}"]`);
                                ...
                                }, commitGrowUnits: ({ growUnitsBefore, growUnitsAfter }) => setPaneSizes({
                                    ...paneSizes,
                                    [panes[index - 1]]: growUnitsBefore,
                                    [pane]: growUnitsAfter,
                                }) })), pane === 'turns' && (_jsx(TurnList, { sessionId: id, view: raw ? 'raw' : 'turns', markdown: markdown, streaming: streaming, compacting: controls.compacting, composerStatus: composerStatus, style: paneStyle('turns') })), pane === 'timeline' && _jsx(Timeline, { sessionId: id, style: paneStyle('timeline') }), pane === 'git' && id && (_jsx(GitPanel, { sessionId: id, refetchSignal: summary?.state ?? '', gitRepos: gitRepos.repos, selectedRepo: gitRepos.selectedRepo, setSelectedRepo: gitRepos.setSelectedRepo, gitReposLoading: gitRepos.loading, gitReposError: gitRepos.error, refreshGitRepos: gitRepos.refresh, onToggleCollapse: () => togglePane('git'), style: paneStyle('git'), paneKey: "git" })), pane === 'attach' && id && (_jsxs("div", { className: "bc-split-pane bc-split-pane-attach", style: paneStyle('attach'), "data-pane": "attach", children: [_jsxs("div", { className: "bc-split-pane-header", children: [_jsx("span", { className: "bc-split-pane-title", children: "Terminal" }), _jsx("span", { className: "bc-spacer" }), _jsx("button", { type: "button", className: "bc-split-collapse-btn", "aria-label": "Hide terminal", title: "Hide terminal", onClick: () => togglePane('attach'), children: "\u00D7" })] }), attach.state.status === 'ready' && (_jsx(BridgeAttach, { sessionId: id, attachToken: attach.state.token })), attach.state.status === 'loading' && (_jsx("div", { className: "bc-git-empty", children: "Connecting\u2026" })), attach.state.status === 'absent' && (_jsx("div", { className: "bc-git-empty", children: "This session has no live terminal. Switch its mode to pty to start one." })), attach.state.status === 'error' && (_jsxs("div", { className: "bc-git-error", children: ["Terminal: ", attach.state.message] }))] })), pane === 'kanban' && id && (_jsx(LinkedKanbanPanel, { sessionId: id, onToggleCollapse: () => togglePane('kanban'), style: paneStyle('kanban'), paneKey: "kanban" }))] }, pane))), panes.length === 0 && (
                    ...
                    _jsx("div", { className: "bc-split-empty", children: _jsx("div", { className: "bc-split-empty-hint", children: "Every pane is hidden. Use the pane toggles above to bring one back." }) }))] }), _jsx(BudgetBanner, { sessionId: id }), id && _jsx(AwaitingYouBanner, { sessionId: id }), minimal && controlsSlot ? createPortal(settingsSheet, controlsSlot) : null, turnsPaneHidden && sessionStatus !== null && (_jsx(SessionStatusLine, { status: sessionStatus, onOpenSession: select })), _jsx(Composer, { sessionId: id, turnRunning: streaming, composer: composer, onFailedAction: setComposerFailedAction })] }));
}
