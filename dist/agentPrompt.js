 ...
 */
export function readAgentPrompt(body) {
    if (!body)
        return null;
 ...
 */
export function stripAgentPrompt(body) {
    if (!body)
        return '';
    const start = body.indexOf(AGENT_PROMPT_OPEN);
    if (start === -1)
 ...
 */
export function writeAgentPrompt(body, prompt) {
...
export function suggestAgentPrompt(args) {
    ...
    if (linkedEmailCount > 0) {
        lines.push(``, `## Context`, `${linkedEmailCount} email${linkedEmailCount === 1 ? '' : 's'} ${linkedEmailCount === 1 ? 'is' : 'are'} attached to this card. Read them before acting — mailstack serves them at /api/messages/{id}?account={account}, and each linked reference is account-qualified as account:message_id.`);
    }
    lines.push(``, `## When you are done`, `1. Do the work described above.`, `2. PATCH http://localhost:8191/api/items/${cardID} with {"status":"done"} and a body update saying what you did.`, `3. Exit cleanly. The kanban-curator moves the card to Done once this session ends.`, `4. If it is blocked or unclear, leave a body update explaining why and exit — the card stays where it is and a human will pick it up.`);
    return lines.join('\n');
}
