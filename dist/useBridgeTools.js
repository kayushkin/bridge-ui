 ...
 */
export function useBridgeTools(instanceID) {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            if (!toolsRes.ok)
                throw new Error(`/tools HTTP ${toolsRes.status}`);
            if (!localsRes.ok)
                throw new Error(`/locals HTTP ${localsRes.status}`);
    ...
    const setGlobal = useCallback(async (toolID, on) => {
        if (!enabled)
            return false;
    ...
    const setForInstance = useCallback(async (toolName, on) => {
        if (!enabled || !instanceID)
            return false;
...
//# sourceMappingURL=useBridgeTools.js.map