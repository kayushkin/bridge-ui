...
import type { FetchFn, HookEvent, Signal } from '../../types'
import { SignalStateOpen, SignalSurfaceChat } from '../../types'

// Data layer for session signals — the canonical record of anything a session
// surfaces to a human (llm-bridge/msg/signal.go, table + read API in
// llm-bridge-server/internal/store/signals.go).
//
// Read path: GET {basePath}/signals?state=open&surface=chat[&session_id=…].
//
// The per-session route GET /sessions/{id}/signals exists too, but this module
// deliberately uses the cross-session route for both the inbox and the
// per-session panel: the per-session route answers 404 for BOTH "this server
// has no signals route" and "no such session", and those two need different
// handling in the UI. /signals only 404s for the first, so the answer is
// unambiguous.
//
// Resolve path: signals minted by the tool producer carry the request_id of the
// hook their question parked, and answering them IS resolving that hook — see
...
async function getSignals(fetchFn: FetchFn, url: string): Promise<SignalsResult> {
  const res = await fetchFn(url)
  if (res.status === 404) return null
  if (!res.ok) throw new Error(`HTTP ${res.status} ${await res.text()}`)
  const body: unknown = await res.json()
  if (!Array.isArray(body)) throw new Error('signals response was not an array')
  return body as Signal[]
}

/** Open chat-surface signals, newest first. Pass a sessionId to scope to one
 * session; omit it for the cross-session inbox. */
export function fetchOpenChatSignals(
  fetchFn: FetchFn,
  basePath: string,
  sessionId?: string,
  limit?: number,
...
export async function fetchOpenSignalsByTodo(
  fetchFn: FetchFn,
  basePath: string,
): Promise<Map<string, Signal[]> | null> {
  ...
  const byTodo = new Map<string, Signal[]>()
  for (const signal of signals) {
    ...
    if (existing) existing.push(signal); else byTodo.set(todoID, [signal])
  }
  return byTodo
}
...
export function fetchOpenSignalsForTodo(
  fetchFn: FetchFn,
  basePath: string,
  todoID: string,
): Promise<SignalsResult> {
...
export function groupSignalsByRequest(signals: Signal[]): SignalRequest[] {
...
async function postResolve(
  ...
  const res = await fetchFn(
    `${basePath}/sessions/${encodeURIComponent(sessionId)}/hooks/${encodeURIComponent(requestId)}/resolve`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) },
  )
  if (!res.ok) throw new Error(`resolve failed: HTTP ${res.status} ${await res.text()}`)
}
...
export function useOpenChatSignals(sessionId?: string, refreshKey?: string | number): UseOpenChatSignals {
...
export function useOpenSignalsByTodo(): Map<string, Signal[]> {
  const { fetch: fetchFn, basePath } = useBridgeConfig()
  const [byTodo, setByTodo] = useState<Map<string, Signal[]>>(new Map())
  const [reloadToken, setReloadToken] = useState(0)
