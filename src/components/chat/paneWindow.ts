 ...
 */
export function windowStartIndex<Block>(
  blocks: Block[],
  budget: number,
  sizeOf: (block: Block) => number,
): number {
  if (blocks.length === 0) return 0
  let counted = 0
  for (let i = blocks.length - 1; i > 0; i--) {
    counted += sizeOf(blocks[i])
    if (counted >= budget) return i
  }
  return 0
}

/** How many units sit above the window — what the "show earlier" control counts. */
export function unitsBeforeWindow<Block>(
  blocks: Block[],
  windowStart: number,
  sizeOf: (block: Block) => number,
): number {
  let counted = 0
  for (let i = 0; i < windowStart && i < blocks.length; i++) counted += sizeOf(blocks[i])
  return counted
}
